...
void cooley_tukey(double complex* X, const size_t N) {
  if (N >= 2) {
    // Splits the array, so the top half are the odd elements and the bottom
    // half are the even ones.
...
void bit_reverse(double complex* X, size_t N) {
  // Bit reverses the array X[] but only if the size of the array is less then
  // 2^32.
  double complex temp;
  unsigned int b;
